...
<?php if ( be_build( get_the_ID(), 'group_cat_b' ) ) {
	if ( ! be_build( get_the_ID(), 'cat_b_bg' ) || ( be_build( get_the_ID(), 'cat_b_bg' ) == 'auto' ) ) {
		$bg = '';
	}
	if ( be_build( get_the_ID(), 'cat_b_bg' ) == 'white' ) {
		$bg = ' group-white';
	}
	if ( be_build( get_the_ID(), 'cat_b_bg' ) == 'gray' ) {
		$bg = ' group-gray';
	}
?>
<div class="g-row g-line group-cat-b<?php echo $bg; ?>" <?php aos(); ?>>
	<div class="g-col">
		<div class="group-cat">
			<?php $categories =  explode(',', be_build( get_the_ID(), 'group_cat_b_id' ) );
				...
				<div class="gr2">
					<div class="gr-cat-box">
						...
						<div class="gr-cat-area">
							<?php if ( be_build( get_the_ID(), 'group_cat_b_top' ) ) { ?>
								...
								<ul class="gr-cat-list" <?php aos_b(); ?>>
									<?php
										$imgb = get_posts( array(
											'posts_per_page' => be_build( get_the_ID(), 'group_cat_b_n' ),
											'post_status'    => 'publish',
											$cat             => $category,
		...
		<?php bu_help( $text = '新闻资讯B', $number = 'group_cat_b_s' ); ?>
	</div>
</div>
<?php } ?>