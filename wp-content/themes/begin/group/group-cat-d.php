...
<?php if ( co_get_option( 'group_cat_d' ) ) {
	if ( ! co_get_option( 'cat_d_bg' ) || ( co_get_option( 'cat_d_bg' ) == 'auto' ) ) {
		$bg = '';
	}
	if ( co_get_option( 'cat_d_bg' ) == 'white' ) {
		$bg = ' group-white';
	}
	if ( co_get_option( 'cat_d_bg' ) == 'gray' ) {
		$bg = ' group-gray';
	}
?>
<div class="g-row g-line group-cat-d<?php echo $bg; ?>" <?php aos(); ?>>
	<div class="g-col">
		<div class="grf-cat-box">
			<?php 
				$catd = ( array ) co_get_option( 'group_cat_d_item' );
		...
		<?php co_help( $text = '公司主页 → 新闻资讯D', $number = 'group_cat_d_s' ); ?>
	</div>
</div>
<?php } ?>