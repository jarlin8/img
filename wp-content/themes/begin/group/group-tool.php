...
<div class="g-row g-line sort" name="<?php echo zm_get_option('tool_s'); ?>" <?php aos(); ?>>
	<div class="g-col">
		<div class="group-tool-box">
			...
			<?php $posts = get_posts( array( 'post_type' => 'any', 'orderby' => 'menu_order', 'order' => 'ASC', 'meta_key' => 'tool_ico', 'numberposts' => '16') ); if($posts) : foreach( $posts as $post ) : setup_postdata( $post ); ?>
			<?php 
			...
			<div class="sx4 edit-buts stool-<?php echo zm_get_option('stool_f'); ?>">
				<div class="group-tool sup bkh" <?php aos_b(); ?>>
					...
					<div class="group-tool-pu bgt">
						<div class="group-tool-ico">
							<?php if ( get_post_meta($post->ID, 'tool_ico_svg', true) ) { ?>
						...
						<?php if ( get_post_meta($post->ID, 'tool_button', true) ) { ?>
							<div class="group-tool-link bgt"><a class="bgt" href="<?php if ( get_post_meta($post->ID, 'tool_url', true) ) { ?><?php echo $tool_url; ?><?php } else { ?><?php the_permalink(); ?><?php } ?>" target="_blank" rel="external nofollow" data-hover="<?php echo $tool_button; ?>"><span><i class="be be-more"></i></span></a></div>
						<?php } ?>
					</div>
				</div>
			</div>
