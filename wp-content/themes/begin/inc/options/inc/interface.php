	...
	static function optionsframework_tabs() {
		...
		foreach ( $options as $value ) {
			if ( @$value['type'] == "heading" ) {
				...
				$menu .= '<a id="options-group-'.  $counter . '-tab" class="nav-tab ' . $class .'" href="' . esc_attr( '#options-group-'.  $counter ) . '">' . esc_html( $value['name'] ) . '</a>';
			}
		}
	...
	static function optionsframework_fields() {
		...
		foreach ( $options as $value ) {
			...
			case 'groupstart':
				...
				$output .= '<div ' . $id . 'class="' . esc_attr( $class ) . '">' . "\n";
				if ( isset($value['name']) ) {
					$output .= '<h4 class="group-heading">' . esc_html( $value['name'] ) . '<span class="heading-arrow show"></span></h4>' . "\n";
				}

				if (zm_get_option('show_box')) {
					$output .= '<div class="group-content group-content-show">';
				} else {
