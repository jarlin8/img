jQuery(document).ready(function($) {
	...
	function optionsframework_add_file(event, selector) {
		...
		if (optionsframework_upload) {
			...
			optionsframework_upload.on('select',
			function() {
				...
				if (attachment.attributes.type == 'image') {
					optionsframework_selector.find('.screenshot').empty().hide().append('<img src="' + attachment.attributes.url + '"><a class="remove-image">×</a>').slideDown('fast');
				}
				...
				optionsframework_selector.find('.remove-image, .remove-file').on('click',
				function() {
	...
	function optionsframework_remove_file(selector) {
		...
		selector.find('.upload-button').on('click',
		function(event) {
			optionsframework_add_file(event, $(this).parents('.section-upload'));
		});
	}

	$('.remove-image, .remove-file').on('click',
	function() {
		optionsframework_remove_file($(this).parents('.section-upload'));
	});
