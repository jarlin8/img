			...
			if ( isset($_POST['search_k']) && $_POST['search_k'] ) {
				...
				if ( preg_match("/$search_k/Ui",$cleankeyword) ) {
					if ($cnt++ % 2) echo '<tr class=alternate>'; else echo '<tr>';
						...
						echo "<td>";
						if ( $zh_CN )       echo " 中文 ";
						if ( $newwindow )   echo " | 新窗口打开";
						if ( $firstonly)    echo " | 仅匹配一个";
						if ( $nofollow )    echo " | Nofollow属性";
						if ( $docomments )  echo " | 匹配评论";
						if ( $ignorecase )  echo " | 忽略大小写";
						if ( $isaffiliate ) echo " | 内链";
						echo "</td>";
...
list($last_link, $last_nofollow, $last_firstonly, $last_newwindow, $last_ignorecase, $last_isaffiliate, $last_docomments, $last_zh_CN, $last_desc) = $values;
	...
	$fields = [
		'keyword' => ['label' => '关键词', 'maxlength' => 400, 'required' => true],
		'link'    => ['label' => '链&nbsp;&nbsp;&nbsp;接', 'maxlength' => 400, 'required' => true],
		'desc'    => ['label' => '描&nbsp;&nbsp;&nbsp;述', 'maxlength' => 500]
	];
				...
				<td class="attributes">
					<?php foreach ( $checkboxes as $name => $label ): ?>
						<label for="<?php echo $name; ?>" style="display: inline-block;padding: 6px 0;">
							<input class="checkbox of-input" type="checkbox" id="<?php echo $name; ?>" name="<?php echo $name; ?>" value="1" <?php if ( ${"last_$name"} ): ?>checked<?php endif; ?> />
							<?php echo $label; ?>
						</label><br />
					<?php endforeach; ?>
				</td>
