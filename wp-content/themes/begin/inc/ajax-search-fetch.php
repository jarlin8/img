...
<!-- list -->
	...
	<?php if( $the_query->have_posts() ) : ?>
		<h4><?php _e( '搜索结果', 'begin' ); ?></h4>
		<ul class="ajax-search-box ms bk ajax-search-result">
...
<!-- img -->
	...
	?>
	<?php if( $the_query->have_posts() ) : ?>
		<section id="picture" class="content-area ajax-search-result grid-cat-<?php echo zm_get_option('img_f'); ?>">
			<h4><?php _e( '搜索结果', 'begin' ); ?></h4>
	...
	<?php if( $the_query->have_posts() ) : ?>
		<section id="primary" class="content-area cms-news-grid-container grid-ajax-search ajax-search-result">
			...
			<?php while( $the_query->have_posts() ): $the_query->the_post(); ?>
				...
				<?php $search = get_the_title(); ?>
				<?php if( stripos("/{$search}/", $a) !== false) { ?>
					<article id="post-<?php the_ID(); ?>" <?php post_class('gl post ms bk'); ?>>
