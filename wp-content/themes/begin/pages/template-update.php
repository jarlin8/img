...
function up_archives_list() {
	if ( !$output = get_option('up_archives_list') ){
		...
		while ( $the_query->have_posts() ) : $the_query->the_post();
			...
			if ($year != $year_tmp) {
				$year = $year_tmp;
				$output .= '<h3 class="year">'. $year .' 年</h3>';
				$output .= '<ul class="mon_list">';
			}
			if ($mon != $mon_tmp) {
				$mon = $mon_tmp;
				$output .= '<span class="mon">'. $mon .'月</span><span class="year-m">'. $year .'年</span><div class="clear"></div>';
...
<div id="archives-up" class="content-area">
	<main id="main" class="be-main site-main" role="main">
		...
		<div class="up-archives">
			<ul class="mon-list bk da ms future-post">
