    ...
    ?>
        <div class="d-flex flex-fill align-items-center mb-4">
            ...
            if($site_n >= 0 && $count < $category_count){
                ...
                echo "<a class='btn-move text-xs' href='$link'>".io_get_option('term_more_text','more+')."</a>";
            } 
            ?>
        </div>
...
function fav_con_tab($category,$parent_term,$is_ajax = false) { 
    ...
    ?>
        ...
        <div class="d-flex flex-fill flex-tab align-items-center">
            ...
            //显示更多按钮，通过js切换href
            if($_link != ''){
                echo "<a class='btn-move tab-move text-xs ml-2' href='$_link'>".io_get_option('term_more_text','more+')."</a>";
            } else {
                echo "<a class='btn-move tab-move text-xs ml-2' href='#' style='display:none'>".io_get_option('term_more_text','more+')."</a>";
            }
            ?>
        </div>
        <!-- tab模式菜单 end -->
