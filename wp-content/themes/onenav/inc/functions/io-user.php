    ...
    foreach ($args as $arg) {
        ...
        if ($bind_href) {
            ...
            if ( $oauth_id ) {
                ...
                $_btn = '<a data-toggle="tooltip" href="javascript:;" openid="' . esc_attr($oauth_id) . '" title="解绑' . $name . '" data-user_id="' . $user_id . '" data-type="' . $type . '" data-action="unbound_open_id" class="btn btn-block btn-lg unbound-open-id vc-l-violet '.$class.'">' . $icon . ' 已绑定' . $name . '</a>';
            } else {
                if ('weixin_gzh' === $type) {
                    $class .= ' qrcode-signin';
                }
                $_btn = '<a data-toggle="tooltip" title="绑定' . $name . '" href="' . esc_url(add_query_arg(array('bind' => $type), $bind_href)) . '" class="btn btn-block btn-lg btn-block btn-outline vc-blue '.$class.'">' . $icon . ' 绑定' . $name . '帐号</a>';
            }
        ...
        if (is_array($list)) {
            foreach ($list as $type) { //聚合登录增加前置io_
                ...
                if ($oauth_info && $oauth_id) {
                    ...
                    $_btn = '<a data-toggle="tooltip" href="javascript:;" openid="' . esc_attr($oauth_id) . '" title="解绑' . $name . '" data-user_id="' . $user_id . '" data-type="io_' . $type . '" data-action="unbound_open_id" class="btn btn-block btn-lg unbound-open-id vc-l-violet ">' . $icon . ' 已绑定' . $name . '</a>';
                } else {
                    $_btn = '<a data-toggle="tooltip" title="绑定' . $name . '" href="' . esc_url(add_query_arg(array('type' => $type), $bind_href)) . '" class="btn btn-block btn-lg btn-block btn-outline vc-blue ' . $class . '">' . $icon . ' 绑定' . $name . '帐号</a>';
                }
                $btn .= '<div class="col-12 col-md-6 my-2">'.$_btn.'</div>';
            }
        }
