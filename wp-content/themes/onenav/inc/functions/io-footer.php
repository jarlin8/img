 ...
 * @LastEditors: iowen
 * @LastEditTime: 2022-07-22 18:03:32
 * @FilePath: \onenav\inc\functions\io-footer.php
 * @Description: 
 */

function io_copyright(){
    $aff = io_get_option('io_aff', true) ? '由<a href="https://www.iotheme.cn/?aff=' . io_get_option('io_id', '') . '" title="一为主题-精品wordpress主题" target="_blank" rel="noopener"><strong> OneNav </strong></a>强力驱动&nbsp' : '';
    if (io_get_option('footer_copyright')) {
        echo io_get_option('footer_copyright') . "&nbsp;&nbsp;" . $aff . io_get_option('footer_statistics');
    } else {
        $copy   = 'Copyright © ' . date('Y') . ' <a href="' . esc_url(home_url()) . '" title="' . get_bloginfo('name') . '" rel="home">' . get_bloginfo('name') . '</a>&nbsp;';
        $icp    = io_get_option('icp', false) ? '<a href="https://beian.miit.gov.cn/" target="_blank" rel="link noopener">' . io_get_option('icp') . '</a>&nbsp;' : '';
        $p_icp  = '';
