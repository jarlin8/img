 ...
 * @LastEditors: iowen
 * @LastEditTime: 2023-03-24 23:02:40
...
function io_book_header(&$is_hide){
    ...
    $html = '<div class="row site-content py-4 py-md-5 mb-xl-5 mb-0 mx-xxl-n5">';
    $html .= '<!-- book封面 -->';
...
function io_book_content_down($name){
    ...
    echo io_get_down_modal($title, $down_list, 'book');
}
/**
 * 期刊名字
 * @return string
...
function io_book_header_info( $booktitle ){
    ...
    $list = '';
    foreach (get_post_meta(get_the_ID(), '_books_data', true) as $value) {
        $list .= '<li class="my-2"><span class="info-title mr-3">' . $value['term'] . '</span>' . $value['detail'] . '</li>';
    }
    $list .= '<li class="my-2"><span class="info-title mr-3">' . __('标签', 'i_theme') . '</span>' . io_get_post_tags($post_id, array('books', 'booktag')) . '</li>';
    $list .= '<li class="my-2"><span class="info-title mr-3">' . __('系列', 'i_theme') . '</span>' . io_get_post_tags($post_id, array('series')) . '</li>';
