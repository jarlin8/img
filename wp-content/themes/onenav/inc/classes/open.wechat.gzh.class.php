 ...
 * @LastEditors: iowen
 * @LastEditTime: 2023-01-29 17:02:04
    ...
    public function responseMsg($msgs = array()) { 
        ...
        if(!empty($callback['MsgType'])){
                ...
                case 'text':
                    if('dyh'===$this->type){
                        if($callback['Content'] == '登录' || $callback['Content'] == '登陆' || $callback['Content'] == '绑定'){
                            ...
                            set_transient( $code, esc_sql($callback['FromUserName']), 5 * MINUTE_IN_SECONDS ); 
            ...
            if ('dyh' === $this->type && (
                    ...
                    ($callback['Event'] == 'click' && $callback['EventKey'] == 'io_ws_login')
            )) {
                $code = rand(100000, 999999);
                set_transient($code, esc_sql($callback['FromUserName']), 5 * MINUTE_IN_SECONDS);
                $content = "验证码：" . $code . "，5分钟内有效，过期后请重新发送“登录”二字获取";
