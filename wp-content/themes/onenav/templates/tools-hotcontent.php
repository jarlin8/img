        ...
        <div class="slider_menu mini_tab" sliderTab="sliderTab" >
            <ul class="nav nav-pills tab-auto-scrollbar menu overflow-x-auto" role="tablist"> 
                ...
                for($i=0; $i< count($hot_list); $i++) { 
                    ...
                    <a class="nav-link ajax-home-hot-list '.($i == 0?'active load':'').'" data-toggle="pill" href="#ct-tab-'.$hot_list[$i]['type'].'-'.$hot_list[$i]['order'].$i.'" data-action="load_hot_post" data-datas="'. esc_attr(json_encode(array('data'=>$hot_list[$i]),JSON_UNESCAPED_UNICODE)) .'">'.$hot_list[$i]['title'].'</a>
                    </li>';
                } 
                ?>
            </ul>
