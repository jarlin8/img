 ...
 * @LastEditors: iowen
 * @LastEditTime: 2023-03-25 04:56:16
...
function iopay_get_auto_ad_html($loc, $class = '', $loc_home='search', $echo = true){
    ...
    if (!in_array($loc, $config['loc']) || ('home' === $loc && $loc_home != $config['loc_home'])) {
        return $html;
    }
    $url = add_query_arg(array('action' => 'pay_auto_ad_modal', 'loc' => $loc), admin_url('admin-ajax.php'));
    $btn = '<a href="'.$url.'" class="btn vc-yellow btn-outline btn-sm py-0 io-ajax-modal-get nofx ml-auto" data-modal_type="overflow-hidden">'.io_option_to_multi_language($config['btn_multi'],'btn_multi').'</a>';
    ...
    $html .= '<div class="text-sm">'.io_option_to_multi_language($config['title_multi'],'title_multi').'</div>';
    $html .= $btn;
    $html .= '</div>';
    if($config['ajax']){
...
function iopay_get_auto_ad_url_list($loc){
    ...
    if (!empty($lists)) {
        foreach ($lists as $val) {
            ...
            $html .= '<a href="' . $val['url'] . '" class="d-flex align-items-center auto-url-list io-radius px-2 py-1" target="_blank" rel="external nofollow" title="' . $val['name'] . '">';
 ...
 * @param mixed $check 取值范围，1：只取已审核，all：含未审核，只有前台显示需要检查
 * @return array
 */
function iopay_get_valid_auto_ad_url($loc, $status = '1', $check = 'all'){
...
function iopay_add_auto_ad_url($url){
    $defaults = array(
        ...
        'limit'     => '',
        'order_num' => '',
        'expiry'    => date('Y-m-d H:i:s', current_time('timestamp') + (5 * 60)), //3分钟后过期，支付成功后更新时间
    );
    if (isset($url['id'])) {
        unset($url['id']);
    }
    ...
    $lists   = iopay_get_valid_auto_ad_url('AHP', 'all'); //站点所有url
    $lists[] = $url;
    update_option('auto_ad_url_list', maybe_serialize($lists), false);
    return true;
}
/**
 * Summary of iopay_auto_loc_is_spare
 * @param mixed $loc
 * @param mixed $count
 * @return void
 */
...
function iopay_update_auto_ad_url($url){
    $defaults = array(
        'status'    => 1,
        'check'     => io_get_option('auto_ad_config', true, 'check') ? 0 : 1,
        'url'       => '',
        'name'      => '',
        'loc'       => '',
        'time'      => '',
    ...
    $lists  = iopay_get_valid_auto_ad_url('AHP', 'all');
    $status = false;
 ...
 * @return mixed
 */
function iopay_get_latest_expiration_time($lists, $loc, $status, $array = false){
    if(empty($lists)){
...
/**
 * Summary of iopay_get_auto_unit_name
 * @param mixed $type
 * @return mixed
 */
 ...
 * @return void
 */
function iopay_check_auto_ad($id){
    $lists = maybe_unserialize(get_option('auto_ad_url_list', array()));
    $count = count($lists);
    $unit  = io_get_option('auto_ad_config', 'hour', 'unit');
    for ($i = 0; $i < $count; $i++) {
        if ($lists[$i]['token'] == $id) {
            if (!empty($lists[$i]['limit'])) {
                if ('hour' === $unit) {
                    ...
                    $lists[$i]['expiry'] = date('Y-m-d H:i:s', strtotime("+{ $lists[$i]['limit']}{$unit}", current_time('timestamp')));
                }
            }
            $lists[$i]['check'] = 1;
        }
...
function iopay_delete_auto_ad($id){
    ...
    $count = count($lists);
    for ($i = 0; $i < $count; $i++) {
        if (strtotime($lists[$i]['expiry']) < (current_time('timestamp')) || $lists[$i]['token'] == $id) {
            //删掉过期资源
            unset($lists[$i]);
        }
    }
