 ...
 * @LastEditors: iowen
 * @LastEditTime: 2023-03-24 23:49:23
...
function iopay_add_order_data($post_data = array()){
    ...
    switch ($order_type) {
        case 'auto_ad_url':
            ...
            $ad_data = array(
                'url'   => !empty($post_data['url']) ? $post_data['url'] : '',
                'name'  => !empty($post_data['url_name']) ? $post_data['url_name'] : '',
                'loc'   => $loc_type,
...
function iopay_pay_auto_ad_modal($loc){
    ...
    $form .= '<input type="text" name="url_name" class="form-control mb-2" placeholder="'.__('请输名称','i_theme').'" value="">';
    $form .= '<input type="text" name="url" class="form-control mb-3" placeholder="'.__('请输URL','i_theme').'" value="">';
    $form .= iopay_get_pay_input($buy_type);
    $form .= '</form>';

    $tips = io_option_to_multi_language($config['tips_multi'],'tips_multi');
    if ($tips) {
        $tips = '<div class="bg-muted text-muted text-xs text-center p-2">'.$tips.'</div>';
    }
    ...
    if(isset($config['check']) && $config['check']){
        $check_tips = '<div class="tips-box vc-l-red btn-block text-xs text-center p-1 mt-3">'.io_option_to_multi_language($config['check_tips_multi'],'check_tips_multi').'</div>';
    }
