...
function be_display_contact_form() {
	...
	$content .= '<div class="notice fd bgt hz">' . __( '带', 'begin' ) . '<i class="be be-star"></i>' . __( '号项必填', 'begin' ) . '</div>';
	$content .= bec_submit();
	$content .= '</form>';
...
function bec_input( $name, $label, $type = 'text', $required = false, $has_label = true ) {
	...
	$content .= "<label class='" . ( $has_label ? 'label' : 'no-label' ) . "' for='$name'><span class='mail-label hz'>" .$label . ( $required ? '<span class="required"><i class="be be-star"></i></span>' : '' ) . "</span></label>";
	$content .= "<input type='$type' id='$name' class='dah bk $name' name='$name' placeholder='' " . ( $required ? 'required' : '' ) . " />";
	$content .= "</div>";
	return $content;
}
...
function bec_textarea( $name, $label, $required = false ) {
	$content  = "<div class='item item-$name'>";
	$content .= "<label class='label' for='$name'><span class='mail-label hz'>" .$label . ( $required ? '<span class="required"><i class="be be-star"></i></span>' : '' ) . "</span></label>";
	$content .= "<textarea id='$name' class='dah bk $name' name='$name' placeholder='' rows='4' " . ( $required ? 'required' : '' ) . "></textarea>";
	$content .= "</div>";
	return $content;
}
