		...
		<div class="user-box">
			<?php if ( get_option( 'show_avatars' ) ) { ?>
				<div class="user-my load">
					...
					<?php } ?>
					<span class="hi-user"><a href="javascript:void(0)"><?php echo $user_identity; ?>，<?php if ( !zm_get_option('user_url') == '' ) { ?><?php _e( '用户中心', 'begin' ); ?><?php } else { ?><?php _e( '您好！', 'begin' ); ?><?php } ?></a></span>
				</div>
			<?php } else { ?>
...
<?php } else { ?>
<div class="m-tou">
	<p><a class="bk" href="<?php echo get_permalink( zm_get_option('tou_url') ); ?>" target="_blank"><i class="be be-editor ri"></i><?php _e( '点击跳转到投稿页面', 'begin' ); ?></a></p>
</div>
<?php } ?>
<?php }