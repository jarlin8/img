        ...
        <?php foreach ($items as $key => $item): ?>           
            ...
            <div class="row-products"<?php if (!$is_shortcode) echo ' style="display:flex;"';?>>
                <div class="<?php if ($is_shortcode) echo 'col-md-2 col-sm-2 ';?>col-xs-12 cegg-image-cell">
                    ...
                    <?php if ($item['img']): ?>
                        <a<?php TemplateHelper::printRel(); ?> target="_blank" href="<?php echo esc_url_raw($item['url']); ?>"><img<?php if (!$is_shortcode) echo ' style="max-height: 70px;"'; ?> src="<?php echo esc_url($item['img']); ?>" alt="<?php echo \esc_attr($item['title']); ?>" /></a>
                    <?php endif; ?>
                </div>
                <?php if ($is_shortcode): ?>
