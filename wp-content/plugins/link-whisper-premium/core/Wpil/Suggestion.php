        ...
        if(isset($_POST['type']) && 'outbound_suggestions' === $_POST['type']){
            ...
            if(empty($suggested_post_ids)){
                ...
                foreach($suggested_posts as $suggested_post){
                    $suggested_post_ids[] = 'post_' . $suggested_post->ID;
                }

                foreach($suggested_terms as $suggested_term){
                    $suggested_post_ids[] = 'term_' . $suggested_term->term_id;
                }
...
//            $unique_keywords = self::getPostUniqueKeywords($target_keywords, $process_key);
            ...
            if(!empty($target_keywords)){
                ...
                foreach($target_keywords as $target_keyword){
                    ...
                    if($in_unstemmed || $in_stemmed) 
                    {
                        ...
                        if($in_unstemmed){
                            $pos = mb_strpos($phrase->text, $target_keyword->keywords);
                            if(Wpil_Keyword::isPartOfWord($phrase->text, $target_keyword->keywords, $pos)){ // TODO: Create a general isPartOfWord function in a toolbox class so we don't have to go reaching across classes for methods
                                continue;
                            }
                        }
    ...
    public static function getTitleWords($post, $target = null, $keyword = null, $count = null, $process_key = 0)
    {
        ...
        if(!empty($target) && $words = get_transient('wpil_inbound_title_words_' . $process_key)){
            return $words;
        }else{
            ...
            // save the title words if this is an Inbound Search
            if(!empty($target)){
...
error_log(print_r($post, true));
    ...
    public static function getSuggestionAnchorWords($text, $words = array(), $return_indexes = false){
        ...
        $word_occurances = array_count_values($stemmed_phrase_words);
        ...
        if(!empty($posts)){
            ...
            foreach($posts as $post){
                ...
                if(!isset($redirected[$post->ID])){
                    ...
                    if($multi_lang && in_array($post->ID, $same_language_posts)){
    ...
    public static function get_inbound_suggested_terms($keyword, $post){
                ...
                empty(Wpil_Settings::get_suggestion_filter('same_category'))) // we're not restricting by category
        {
            ...
            if ($post->type == 'post') {
                if (Wpil_Settings::translation_enabled()) {
                    $multi_lang = true;
                    $same_language_posts = Wpil_Post::getSameLanguageTerms($post->id);
                }
            }

            if($multi_lang){
                $language_ids .= " AND t.term_id IN (" . implode(',', $same_language_posts) . ")";
            }
            ...
            if(!empty($same_language_posts) && $multi_lang){
                ...
                foreach($id_batches as $batch){
                    ...
                    $batch_ids = $wpdb->get_results("SELECT t.term_id FROM {$wpdb->prefix}term_taxonomy tt LEFT JOIN {$wpdb->prefix}terms t ON tt.term_id = t.term_id WHERE tt.taxonomy IN ('" . implode("', '", $taxonomies) . "') {$content} {$include} $exclude");
                ...
                $results = $wpdb->get_results("SELECT t.term_id FROM {$wpdb->prefix}term_taxonomy tt LEFT JOIN {$wpdb->prefix}terms t ON tt.term_id = t.term_id WHERE tt.taxonomy IN ('" . implode("', '", $taxonomies) . "') {$content} {$language_ids} $exclude");
            }

            foreach ($results as $term) {
                if(!$multi_lang || ($multi_lang && in_array($term->term_id, $same_language_posts, true)) ){
                    $terms[$term->term_id] = (object) array('term_id' => $term->term_id);
                }
            }
    ...
    public static function getPostsFromAlternateLocations($posts, $keyword, $exclude_ids){
        ...
        if(!empty($fields)){
            ...
            if(!empty($words)){
                $post_types_p = Wpil_Query::postTypes('p');
                $statuses_query_p = Wpil_Query::postStatuses('p');
                $results = $wpdb->get_results("SELECT DISTINCT m.post_id AS ID FROM {$wpdb->posts} p INNER JOIN {$wpdb->postmeta} m ON p.ID = m.post_id WHERE m.meta_key IN ('" . implode("', '", $fields) . "') {$age_string} {$post_types_p} {$statuses_query_p} AND (meta_value LIKE '%" . implode("%' OR meta_value LIKE '%", $words) . "%')");
