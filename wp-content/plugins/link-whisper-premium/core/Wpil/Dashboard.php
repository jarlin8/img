...
class Wpil_Dashboard
{
     ...
     * @return array
     */
    public static function getDomainsData($per_page, $page, $search, $search_type = 'domain', $show_attributes = true)
    {
        global $wpdb;
        $domains = [];
        if(!empty($search)){
        ...
        foreach ($result as $link) {
            ...
            if (empty($domains[$host])) {
                ...
                if($show_attributes){
                    $dat['attributes'] = Wpil_Settings::get_active_link_attrs($host, true);
                }
                $domains[$host] = $dat;
            }
