...
//            $anchor = addslashes($anchor);
            ...
            if(!empty($link_structure)){
                ...
                if(false !== strpos($link_structure, '%postname%')){
                    ...
                    if(!empty($name)){
                        ...
                        $dat = $wpdb->get_col($wpdb->prepare("SELECT `ID` FROM {$wpdb->posts} WHERE `post_name` = %s {$post_types} LIMIT 1", $name));
        ...
        while($position_start !== false){
                ...
                !self::check_if_inside_json($content, $position_start, $position_end))
            {
                ...
                if($changed_text !== $replace){
                    ...
                    if($position_end + 1 < mb_strlen($content)){
    ...
    public static function check_if_inside_json($content, $position_start, $position_end){
        ...
        // first find any opening JSON tag
        $open = mb_strrpos($content, '{"', $position_start - $length);
        $close = mb_strrpos($content, '"}', $position_start - $length);

        // if there is an opening tag and it's closer than any closing tag
        if($open > $close){
            // search for the other end of the string to find the closing tag
            $close = mb_strpos($content, '"}', $position_end);
            $maybe_open = mb_strpos($content, '{"', $position_end);
            
            // if there's a closer closing tag than an opening tag
            if($close > $maybe_open){
                // extract the string and see what happens when we through it through json_decode
                $data = mb_substr($content, $open, $close - $open + 2); // +2 because close counts upto "} but doesn't include them
                $data = json_decode($data);
                // if parsing the data didn't result in NULL
                if(!empty($data)){
                    // return true because this is inside JSON
                    return true;
                }
            }
        }

        // if we made it here, the text shouldn't be json
        return false;
    }
