...
class Support {
	...
	public function __construct() {
		...
		add_action( 'aawp_admin_menu', [ &$this, 'add_admin_menu' ], 60 );
	 ...
	 * @return      void
	 */
	public function add_admin_menu( $menu_slug ) {
		...
		<table class="widefat">
			...
			<tr class="alternate">
				...
				<?php if ( ! $disable_wp_cron ) { ?>
					<td>
					<?php
					$cache_next_update = wp_next_scheduled( 'aawp_wp_scheduled_events' );
					echo ( $cache_next_update ) ? aawp_datetime( $cache_next_update ) : 'N/A';
					?>
					</td>
				<?php } else { ?>
		...
		<table class="widefat">
			...
			<tr>
				...
				<td>
					...
					<?php
					if ( isset( $this->curl['version'] ) ) {
						echo ' (Version ' . $this->curl['version'] . ')';}
					?>
				</td>
			</tr>
			<tr class="alternate">
