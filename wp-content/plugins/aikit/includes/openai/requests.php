...
function aikit_rest_openai_get_available_models($for = 'text', $from_cache = false, $is_checking_api_key = false) {
    ...
    if (!empty($aikit_openai_models_cache)) {
        return aikit_apply_model_filters($aikit_openai_models_cache, $for);
    }

    $cached_models = get_option('aikit_openai_models_cache');
    if (!empty($cached_models) && $from_cache) {
        return aikit_apply_model_filters($cached_models, $for);
    }

    $client = new \AIKit\Dependencies\GuzzleHttp\Client();

    try {
        $res = $client->request('GET', 'https://geminiapi.ifttt.fun/v1/models', [
            'headers' => [
        ...
        return aikit_get_default_model_list();
    }
    ...
    return aikit_apply_model_filters($models, $for);
}

function aikit_get_default_model_list() {
    return [
        'gpt-3.5-turbo',
        'gpt-3.5-turbo-0301',
        'text-davinci-003',
...
function aikit_apply_model_filters ( $models, $for ) {
    if ($for == 'text') {
        $models = aikit_filter_models_for_text($models);
    } else if ($for == 'embeddings') {
        $models = aikit_filter_models_for_embeddings($models);
    } else if ($for == 'images') {
        $models = aikit_filter_models_for_images($models);
    } else if ($for == 'tts') {
        $models = aikit_filter_models_for_tts($models);
    } else if ($for == 'stt') {
        $models = aikit_filter_models_for_stt($models);
    } else if ($for == 'fine-tuning') {
        $models = aikit_filter_models_for_fine_tuning($models);
    }

    return $models;
}

function aikit_filter_models_for_text($models) {
    $prefixes = [
        'gpt',
        'davinci',
        'curie',
    ...
    return aikit_filter_models($models, $prefixes);
}

function aikit_filter_models_for_embeddings($models)
{
    $prefixes = [
        'text-embedding',
    ];
...
function aikit_filter_models_for_fine_tuning($models)
{
    ...
    return array_merge($fine_tuning_models, aikit_filter_models($models, $prefixes));
}

function aikit_filter_models_for_images($models)
{
    $prefixes = [
        'dall-e',
    ];
...
function aikit_filter_models_for_tts($models) {
    $prefixes = [
        'tts'
    ];
...
function aikit_filter_models_for_stt($models) {
    $prefixes = [
        'whisper'
    ];
...
function aikit_filter_models ($models, $prefixes) {
    $filteredModels = [];
...
function aikit_rest_openai_do_request ( $data, $type, $language ) {
	...
	$theoreticalMaxTokensToGenerate = aikit_get_max_tokens_for_model($model) - $tokenizer->count($prompt) - $tokenizer->count($text);
    ...
    try {
        ...
        $res = $client->request('POST', aikit_get_openai_text_completion_endpoint($model), [
            'body' => aikit_build_text_generation_request_body($model, $prompt, $actualMaxTokensToGenerate, $temperature, $completion_stop_sequence),
            'headers' => [
                'Authorization' => 'Bearer ' . get_option('aikit_setting_openai_key'),
                'Content-Type' => 'application/json',
            ],
...
function aikit_get_openai_text_completion_endpoint ($model) {
    if (strpos($model, 'gpt-3.5-turbo') === 0 || strpos($model, 'gpt-4') === 0) {
        return 'https://geminiapi.ifttt.fun/v1/chat/completions';
    }

    return 'https://geminiapi.ifttt.fun/v1/completions';
}

function aikit_build_text_generation_request_body ($model, $prompt, $maxTokensToGenerate, $temperature = 0.7, $stopSequence = null) {
    if (strpos($model, 'gpt-3.5-turbo') === 0 || strpos($model, 'gpt-4') === 0) {
        $messages = [];
...
function aikit_openai_text_generation_request($prompt, $maxTokens, $temperature = 0.7, $model = null)
{
    ...
    try {
        ...
        $res = $client->request('POST', aikit_get_openai_text_completion_endpoint($model), [
            'body' => aikit_build_text_generation_request_body($model, $prompt, $maxTokens, $temperature, $completion_stop_sequence),
            'headers' => [
                'Authorization' => 'Bearer ' . get_option('aikit_setting_openai_key'),
                'Content-Type' => 'application/json',
            ],
...
function aikit_openai_chat_generation_request($prompt, $maxTokens, $context = '', $context_messages = [], $temperature = 0.7)
{
    ...
    try {
        ...
        $res = $client->request('POST', aikit_get_openai_text_completion_endpoint($model), [
            'body' => aikit_build_chat_generation_request_body($model, $prompt, $maxTokens, $context, $context_messages, $temperature, $completion_stop_sequence),
            'headers' => [
                'Authorization' => 'Bearer ' . get_option('aikit_setting_openai_key'),
                'Content-Type' => 'application/json',
            ],
...
function aikit_build_chat_generation_request_body ($model, $prompt, $maxTokensToGenerate, $context = '', $context_messages = [], $temperature = 0.7, $stopSequence = null) {
    if (strpos($model, 'gpt-3.5-turbo') === 0 || strpos($model, 'gpt-4') === 0) {
        $messages = [];
