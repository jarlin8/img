...
(function (window) {
    ...
    SmartManager.prototype.init = function () {
		...
		this.calendly_link = sm_beta_params?.calendly_link || '';
		...
		window.smart_manager.grid_height = document.body.clientHeight * 0.66;
	...
	SmartManager.prototype.loadNavBar = function () {
		...
		let sm_bottom_bar = "<div id='sm_bottom_bar' class='mt-4' style='width:" + window.smart_manager.grid_width + "px;'>" +
		...
		if (sm_msg) {
			...
			jQuery('.wrap > .sm_design_notice').css('display', 'flex')
		}
			...
			afterGetColHeader: function(col, TH) {
				...
				if (headerText === 'sm_action_header') {
					...
					if (window.smart_manager.currentVisibleColumns && window.smart_manager.currentVisibleColumns[col]) {
						...
						// Add data-column-key attribute to the colHeader span for test automation
						const colHeader = TH.querySelector('.colHeader');
						if (colHeader && colData) {
							colHeader.setAttribute('data-column-key', window.smart_manager.getColumnDataKey(colData));
						}
	...
	SmartManager.prototype.event_handler = function () {
		...
		jQuery(document).off('change', '#sm_dashboard_select').on('change', '#sm_dashboard_select', function () {
			...
			.off('click', '#sm-custom-view-btn, #sm-custom-view-active-btn').on('click', '#sm-custom-view-btn, #sm-custom-view-active-btn', function (e) {
				e.preventDefault();
				e.stopPropagation();
				const panel = jQuery('#sm-custom-views-panel');
				const isExpanded = !panel.hasClass('hidden');
				panel.toggleClass('hidden');
				jQuery('#sm-custom-view-btn').attr('aria-expanded', !isExpanded);
			})

			// Close custom views panel when clicking outside
			.off('click.smCustomViewPanel').on('click.smCustomViewPanel', function (e) {
				if (!jQuery(e.target).closest('#sm-custom-view-btn, #sm-custom-view-active-btn, #sm-custom-views-panel').length) {
	...
	SmartManager.prototype.renderMultilistValuesInGrid = function (values_str, td, addTitleAttr = true, bgColor = '', color = '') {
		...
		if(options){
			options.forEach(category => {
				badgesDiv += `<div class="inline-block ${!Array.from(td.classList).some(cls => cls.startsWith('sm_beta_select_')) ? 'text-sm-base-foreground' : ''} px-2 py-0.5 rounded-lg text-xs leading-4  whitespace-nowrap mt-1 mr-1 ${!color ? 'border border-sm-base-border font-normal' : 'font-medium'}" style="background-color: ${bgColor ? bgColor : '#fff'}; ${color ? `color:${color} !important` : ''}">${ window.smart_manager.escapeHtmlAttr(category)}</div>`;
			});
		}
		td.innerHTML = `<div style="overflow:hidden;max-height:${String(window.smart_manager.rowHeight).includes('px') ? window.smart_manager.rowHeight : window.smart_manager.rowHeight+'px'}" ${addTitleAttr ? `title="${ window.smart_manager.escapeHtmlAttr(td.innerText)}"` : ""}>${badgesDiv}</div>`;
		return td;
	}
	 ...
	 * Prevents XSS attacks where malicious input containing quotes or HTML
	 * could break out of attributes (e.g., title="...") and inject event handlers.
	 * 
	 * @param {string} str - The string to escape
	 * @returns {string} The escaped string safe for HTML attributes
	 */
	SmartManager.prototype.escapeHtmlAttr = function(str = '') {
...
(function(Handsontable){
	...
	function customNumericTextEditor(query, callback) {
	  ...
	  let dateTimeEditor = Handsontable.editors.TextEditor.prototype.extend(),
        ...
        function numericRenderer(hotInstance, td, row, col, prop, value, cellProperties) {
            td.setAttribute('data-column', window.smart_manager.getColumnDataKey(prop));
		    ...
		    if(!value || value === '' || value == null || value === 0 || value === 0.00 || value === '0' || value === '0.00' ) {
		    	...
		    	td.innerHTML = '<div title="'+  window.smart_manager.escapeHtmlAttr(td.innerText) +'" class="wrapper">' + td.innerHTML + '</div>';
		    }
	  	...
	  	function customTextRenderer(hotInstance, td, row, col, prop, value, cellProperties) {
			...
			if (['_sale_price', '_regular_price', '_price'].some(field => prop.includes(field)) && value && value.length) {
				...
				td.innerHTML = '<div title="'+  window.smart_manager.escapeHtmlAttr(td.innerText) +'" class="wrapper sm-price-cell">' + ( (typeof wc === 'object' && wc.wcSettings && wc.wcSettings.CURRENCY && wc.wcSettings.CURRENCY.symbol) ? wc.wcSettings.CURRENCY.symbol : '' ) + td.innerHTML + '</div>';
			} else {
				td.innerHTML = '<div title="'+  window.smart_manager.escapeHtmlAttr(td.innerText) +'" class="wrapper">' + td.innerHTML + '</div>';
			}
		    return td;
	  	...
	  	function customCheckboxRenderer(hotInstance, td, row, col, prop, value, cellProperties) {
		    ...
		    // Wrap existing elements without destroying event listeners
		...
		function taskActionsRenderer(hotInstance, td, row, col, prop, value, cellProperties) {
			...
			try {
				...
				let html = items.slice(0, maxVisible).map(i => {
					...
					return `<span class="items-center gap-1 px-2 py-0.5 rounded text-xs"><span class="font-medium">${ window.smart_manager.escapeHtmlAttr(i.field || '')}</span><span> ${ window.smart_manager.escapeHtmlAttr(i.operator || '')}</span><span class="font-medium"> ${ window.smart_manager.escapeHtmlAttr(v)}</span></span>`;
				}).join(' ');
				if (hiddenCount > 0) html += `<span class="inline-flex items-center h-5 bg-white border border-[#E5E5E5] px-1.5 py-0.5 rounded-lg text-xs ml-1" title="${ window.smart_manager.escapeHtmlAttr(items.slice(maxVisible).map(fmt).join(' | '))}">+${hiddenCount}</span>`;
				td.classList.add('pl-0');
				td.innerHTML = `<div class="wrapper inline-flex items-center gap-1 pl-0" title="${ window.smart_manager.escapeHtmlAttr(items.map(fmt).join(' | '))}">${html}</div>`;
			} catch (e) {
	...
	SmartManager.prototype.getCustomViewsForDashboard = function() {
		let currentKey = window.smart_manager.dashboardKey;
		const views = [];
		...
		// If on a custom view, get the parent post type to show sibling views
		if (window.smart_manager.isCustomView) {
			const currentViewSlug = window.smart_manager.getViewSlug(window.smart_manager.dashboardName);
			currentKey = viewPostTypes[currentViewSlug] || currentKey;
		}
	...
	SmartManager.prototype.generateCsvExport = function(data = {}) {
		...
		// Handle scheduled export via AJAX
		...
		// Check if batch export should be used:
		let batchExportThreshold = parseInt(sm_beta_params.batchExportThreshold) || 50;
		let selectedIds = window.smart_manager.getSelectedKeyIds() || [];
		//check if use batch export.
		if ( ( window.smart_manager.sm_beta_pro == 1 ) && ( ( ( 'entire_store' === params.storewide_option || 'entire_store_stock_cols' === params.storewide_option ) && ( parseInt(window.smart_manager.totalRecords) > batchExportThreshold ) ) || ( selectedIds.length > batchExportThreshold ) ) ) {
			params.use_batch_export  = true;
			// Add filtered results flag.
			if (window.smart_manager.isFilteredData()) {
				params.filteredResults = 1;
			}

			// Add date params.
			if (window.smart_manager.date_params) {
				if (window.smart_manager.date_params.hasOwnProperty('date_filter_params')) {
					params.date_filter_params = window.smart_manager.date_params['date_filter_params'];
				}
				if (window.smart_manager.date_params.hasOwnProperty('date_filter_query')) {
					params.date_filter_query = window.smart_manager.date_params['date_filter_query'];
				}
			}

			// Make AJAX request for batch export
			window.smart_manager.sendRequest({ data: params, data_type: 'json', showLoader: false }, function(response) {});
			setTimeout(() => {
				window.smart_manager.hideFloatingActionBar();
				window.smart_manager.showProgressDialog(_x('Export CSV', 'progressbar modal title', 'smart-manager-for-wp-e-commerce'));
				if (typeof sa_background_process_heartbeat === "function") {
					sa_background_process_heartbeat(1000, 'export_csv', window.pluginKey);
				}
			}, 1000);
			return;
		}
		// Use direct download for smaller exports.
		window.smart_manager.triggerCsvDownload(params, viewSlug);
	}

	// Direct CSV export via URL redirect (for smaller exports < 50 records)
	SmartManager.prototype.triggerCsvDownload = function(params, viewSlug) {
	...
	/**
	 * Get clean column key for data-column attribute
	 * Removes _meta_value__<key> suffix from postmeta keys
	 * Example: postmeta_meta_key__variation_description_meta_value__variation_description -> postmeta_meta_key__variation_description
	 */
	SmartManager.prototype.getColumnDataKey = function(prop) {
		if (typeof prop !== 'string') return prop;
		if (prop.includes('postmeta_meta_key__') && prop.includes('_meta_value__')) {
			return prop.replace(/_meta_value__[^_].*$/, '');
		}
		return prop;
	}
