...
if ( ! class_exists( 'StoreApps_Upgrade_4_3' ) ) {

	/**
	 * Main class for StoreApps Upgrade
	 */
	class StoreApps_Upgrade_4_3 {
		...
		public function add_quick_help_widget() {
			...
			if ( ! class_exists( 'StoreApps_Cache' ) ) {
				include_once plugin_dir_path( __FILE__ ) . 'class-storeapps-cache.php';
			}
		...
		public function connect_storeapps_notification() {
			...
			if ( $sa_is_page_for_notifications && ! in_array( strtolower( $this->sku ), array( 'sm', 'so', 'bvm', 'saff', 'se', 'afw' ), true ) ) {
		...
		public function check_store_connection() {
			$sa_is_page_for_notifications = apply_filters( 'sa_is_page_for_notifications', false, $this );
			if ( ! $this->is_storeapps_onboarding() && true === $sa_is_page_for_notifications && in_array( strtolower( $this->sku ), array( 'sm', 'so', 'bvm', 'saff', 'se', 'afw' ), true ) ) {
				$access_token = get_option( '_storeapps_connector_access_token' );
				$token_expiry = get_option( '_storeapps_connector_token_expiry' );
				if ( empty( $access_token ) || ( ! empty( $token_expiry ) && time() > $token_expiry ) ) {
		...
		public function is_storeapps_onboarding() {
			...
			if ( in_array( 'administrator', wp_get_current_user()->roles, true ) && 'storeapps' === $get_page && 'onboard' === $get_tab ) {
				return true;
			}
			return false;
		}
					...
					jQuery(function(){
						...
						jQuery.ajax({
							...
							data: {
								...
								code: '<?php echo esc_js( $args['code'] ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>',
