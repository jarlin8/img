...
class Update implements Model_Interface , Deactivatable_Interface {
    ...
    */
    ...
    public function ping_for_new_version() {
        ...
        $license_activated = is_multisite() ? get_site_option( Plugin_Constants::OPTION_LICENSE_ACTIVATED ) : get_option( Plugin_Constants::OPTION_LICENSE_ACTIVATED );
        ...
        $retrieving_update_data = is_multisite() ? get_site_option( Plugin_Constants::OPTION_RETRIEVING_UPDATE_DATA ) : get_option( Plugin_Constants::OPTION_RETRIEVING_UPDATE_DATA );

		if ( $retrieving_update_data === 'yes' )
			return;

        $update_data = is_multisite() ? get_site_option( Plugin_Constants::OPTION_UPDATE_DATA ) : get_option( Plugin_Constants::OPTION_UPDATE_DATA );
        ...
        if ( !empty( $response ) ) {
            ...
            if ( !empty( $response ) && property_exists( $response , 'version' ) ) {
				...
				if ( ( !$update_data && version_compare( $response->version , $installed_version , '>' ) ) ||
                    ...
                    if ( is_multisite() )
                        update_site_option( Plugin_Constants::OPTION_RETRIEVING_UPDATE_DATA , 'yes' );
                    else
                        update_option( Plugin_Constants::OPTION_RETRIEVING_UPDATE_DATA , 'yes' );

                    // Fetch software product update data
                    if ( is_multisite() )
                        $this->_fetch_software_product_update_data( get_site_option( Plugin_Constants::OPTION_ACTIVATION_EMAIL ) , get_site_option( Plugin_Constants::OPTION_LICENSE_KEY ) , home_url() );
                    else
    ...
    private function _fetch_software_product_update_data( $activation_email , $license_key , $site_url ) {
        ...
        if ( !empty( $result ) ) {
            ...
            if ( !empty( $result ) && $result->status === 'success' && !empty( $result->software_update_data ) ) {
                ...
                if ( is_multisite() )
                    update_site_option( Plugin_Constants::OPTION_UPDATE_DATA , $result->software_update_data );
                else
                ...
                if ( !empty( $result ) && $result->status === 'fail' && isset( $result->error_key ) && $result->error_key === 'invalid_license' ) {
                    ...
                    // Invalid license
                    if ( is_multisite() )
                        update_site_option( Plugin_Constants::OPTION_LICENSE_ACTIVATED , 'no' );
                    else
    ...
    public function inject_plugin_update_info( $result , $action , $args ) {
        ...
        if ( $license_activated === 'yes' && $action == 'plugin_information' && isset( $args->slug ) && $args->slug == 'thirstyaffiliates-pro' ) {

            $software_update_data = is_multisite() ? get_site_option( Plugin_Constants::OPTION_UPDATE_DATA ) : get_option( Plugin_Constants::OPTION_UPDATE_DATA );
