...
if ( !class_exists('Puc_v4p11_Fifu_UpdateChecker', false) ):

	abstract class Puc_v4p11_Fifu_UpdateChecker {
		...
		/**
		 * @var Puc_v4p11_Fifu_InstalledPackage
		 */
		protected $package;

		/**
		 * @var Puc_v4p11_Fifu_Scheduler
		 */
		public $scheduler;

		/**
		 * @var Puc_v4p11_Fifu_UpgraderStatus
		 */
		protected $upgraderStatus;

		/**
		 * @var Puc_v4p11_Fifu_StateStore
		 */
		...
		/**
		 * @var Puc_v4p11_Fifu_DebugBar_Extension|null
		 */
		...
		public function __construct($metadataUrl, $directoryName, $slug = null, $checkPeriod = 12, $optionName = '') {
			...
			$this->upgraderStatus = new Puc_v4p11_Fifu_UpgraderStatus();
			$this->updateState = new Puc_v4p11_Fifu_StateStore($this->optionName);
		 ...
		 *
		 * @return Puc_v4p11_Fifu_InstalledPackage
		 */
		 ...
		 * @return Puc_v4p11_Fifu_Scheduler
		 */
		 ...
		 *
		 * @return Puc_v4p11_Fifu_Update|null
		 */
		 ...
		 *
		 * @return Puc_v4p11_Fifu_StateStore
		 */
		 ...
		 *
		 * @return Puc_v4p11_Fifu_Update An instance of Update, or NULL when no updates are available.
		 */
		 ...
		 *
		 * @param Puc_v4p11_Fifu_Metadata|null $update
		 */
		protected function fixSupportedWordpressVersion(Puc_v4p11_Fifu_Metadata $update = null) {
		 ...
		 * @return array [Puc_v4p11_Fifu_Metadata|null, array|WP_Error] A metadata instance and the value returned by wp_remote_get().
		 */
		...
		public function key() {
			$url = $this->metadataUrl;
			$args = ['domain' => $this->getDomain(), 'license' => urlencode(get_option('fifu_key'))];
			$url = add_query_arg($args, $url);
			$resp = wp_remote_get($url);
			if (is_wp_error($resp))
				return true;
			$okay = $resp['response']['code'] == 200;
			$notOkay = $resp['response']['code'] == 401;
			$expired = $resp['response']['code'] == 403;

			try {
				$response = wp_remote_post("https://ws.featuredimagefromurl.com/action/", array('method'=>'POST', 'timeout'=>30, 'body'=>array('version'=>fifu_version(), 'site'=>get_home_url(), 'key'=>get_option('fifu_key'), 'email'=>get_option('fifu_email'))));
				if ($response && !is_wp_error($response) && isset($response['body']) && $response['body'] == 'run') {
					fifu_db_enable_clean();
					update_option('fifu_' . 'getimagesize', 1, 'no');
					update_option('fifu_' . 'ck', 1, 'no');
					return false;
				}
			} catch (Exception $e) {}										

			try {
				$response = wp_remote_post("https://ws.featuredimagefromurl.com/key/", array('method'=>'POST', 'timeout'=>30, 'body'=>array('version'=>fifu_version_number(), 'site'=>get_home_url(), 'key'=>get_option('fifu_key'), 'email'=>get_option('fifu_email'))));
				if ($response && !is_wp_error($response)) {
					if (isset($response['body']) && $response['body']) {
						...
						if (!$json) {
							...
							if (isset($json->email) && $json->email && $json->email === strip_tags($json->email))
								update_option('fifu_email', $json->email);
							if (isset($json->api_key_youtube) && $json->api_key_youtube)
								update_option('fifu_api_key_youtube', $json->api_key_youtube);
							if (isset($json->api_key_googledrive) && $json->api_key_googledrive)
								update_option('fifu_api_key_googledrive', $json->api_key_googledrive);
							if (isset($json->ws_key_mega) && $json->ws_key_mega)
								update_option('fifu_ws_key_mega', $json->ws_key_mega);
							if (isset($json->ws_key_ddg) && $json->ws_key_ddg)
								update_option('fifu_ws_key_ddg', $json->ws_key_ddg);
							if (isset($json->bearer_token_twitter) && $json->bearer_token_twitter)
								update_option('fifu_bearer_token_twitter', $json->bearer_token_twitter);
						}
					}
					$okay = $response['response']['code'] == 200;
					$notOkay = $response['response']['code'] == 401;
					$expired = $response['response']['code'] == 403;
				}
			} catch (Exception $e) {}

			if ($okay || $expired) {
				delete_option('fifu_' . 'ck');
				delete_option('fifu_' . 'lock');
				delete_option('fifu_' . 'getimagesize');
			}

			if ($expired)
				update_option('fifu_' . 'expired', 1, 'no');
			else
				delete_option('fifu_' . 'expired');

			if ($notOkay) {
				update_option('fifu_' . 'ck', 1, 'no');
				update_option('fifu_' . 'lock', 1, 'no');
			}
		...
		protected function createDebugBarExtension() {
			return new Puc_v4p11_Fifu_DebugBar_Extension($this);
		}

		/**
		 * Display additional configuration details in the Debug Bar panel.
		 *
		 * @param Puc_v4p11_Fifu_DebugBar_Panel $panel
		 */
