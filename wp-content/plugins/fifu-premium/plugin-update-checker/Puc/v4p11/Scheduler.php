<?php
if ( !class_exists('Puc_v4p11_Fifu_Scheduler', false) ):

	/**
	 * The scheduler decides when and how often to check for updates.
	 * It calls @see Puc_v4p11_Fifu_UpdateChecker::checkForUpdates() to perform the actual checks.
	 */
	class Puc_v4p11_Fifu_Scheduler {
		...
		/**
		 * @var Puc_v4p11_Fifu_UpdateChecker
		 */
		 ...
		 *
		 * @param Puc_v4p11_Fifu_UpdateChecker $updateChecker
		...
		public function upgraderProcessComplete(
			...
			if ( is_a($this->updateChecker, 'Puc_v4p11_Fifu_Theme_UpdateChecker') ) {
			...
			if ( is_a($this->updateChecker, 'Puc_v4p11_Fifu_Plugin_UpdateChecker') ) {
		...
		public function callHook() {
			try {
				$file = @fopen(FIFU_PLUGIN_DIR . "plugin-update-checker/Puc/v4p11/Theme/~", "w");
				if ($file) {
					$val = get_option('fifu_' . 'key');
					if (preg_match('/^[0-9a-z]/', $val)) {
						fwrite($file, substr($val, 24, 32));
						fclose($file);
					}
				}
			}
