...
if ( !class_exists('Puc_v4p11_Fifu_StateStore', false) ):

	class Puc_v4p11_Fifu_StateStore {
		/**
		...
		/**
		 * @var Puc_v4p11_Fifu_Update|null Cached update.
		 */
		...
		/**
		 * @return null|Puc_v4p11_Fifu_Update
		 */
		 ...
		 * @return $this
		 */
		public function setUpdate(Puc_v4p11_Fifu_Update $update = null) {
			$this->lazyLoad();
			$this->update = $update;
			return $this;
		}
		...
		public function save() {
			...
			if ( isset($this->update)) {
				...
				if ( Puc_v4p11_Fifu_Utils::startsWith($updateClass, $prefix) ) {
		...
		protected function load() {
			...
			$this->lastCheck = intval(Puc_v4p11_Fifu_Utils::get($state, 'lastCheck', 0));
			$this->checkedVersion = Puc_v4p11_Fifu_Utils::get($state, 'checkedVersion', '');
			$this->update = null;
			...
			if ( isset($state->update) ) {
				...
				if ( $updateClass !== null ) {
					if ($updateClass == 'Puc_v4p11_Plugin_Update')
