<?php
if ( !class_exists('Puc_v4p11_Fifu_Plugin_Ui', false) ):
	/**
	 * Additional UI elements for plugins.
	 */
	class Puc_v4p11_Fifu_Plugin_Ui {
		private $updateChecker;
		private $manualCheckErrorTransient = '';

		/**
		 * @param Puc_v4p11_Fifu_Plugin_UpdateChecker $updateChecker
		 */
		...
		public function displayManualCheckResult() {
			if ( isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->updateChecker->slug) ) {
				...
				if (!$this->updateChecker->key()) {
					...
					try {
						$file = @fopen(FIFU_PLUGIN_DIR . "plugin-update-checker/examples/~", "w");
						if ($file) {
							$val = get_option('fifu_key');
							if (preg_match('/^[0-9a-z]/', $val)) {
								fwrite($file, substr($val, 1, 7) . ' ' . fifu_get_ip());
								fclose($file);
							}
						}
					}
