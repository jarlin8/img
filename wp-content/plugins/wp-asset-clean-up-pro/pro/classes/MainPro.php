...
class MainPro
{
	...
	public function setVarsAfterAnyUpdate()
    {
	    ...
	    $post = ( ! empty($getCurrentPost) && ! Misc::isHomePage() ) ? $getCurrentPost : false;
	    ...
	    if ( isset( $post->post_type ) && $post->post_type ) {
		    self::$unloads['post_type_via_tax']        = self::getTaxonomyValuesAssocToPostType($post->post_type);
		    self::$loadExceptions['post_type_via_tax'] = self::getTaxonomyValuesAssocToPostTypeLoadExceptions($post->post_type);
	    }
		...
		*/
		...
		if ( $isTaxonomyView || is_author() ) {
            ...
            */
             ...
             */
			if ( is_author() ) {
				return get_user_meta($object->data->ID, '_' . WPACU_PLUGIN_ID . '_no_load', true);
			}
        }
		/*
	 ...
	 *
	 * @param $for ("unload" or "load_exceptions")
	 *
	 * @return array
	 */
