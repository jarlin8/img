...
class StoragePageController extends AbstractMenuPageController
{
    ...
    public function actionEditSave()
    {
        ...
        if ($storage->storage_type == DUP_PRO_Storage_Types::Local) {
            ...
            $core_dir_used = $storage->is_path_in_core_dirs();
            if ($core_dir_used) {
                $error_message = __('Local storage inside core WordPress directories are not allowed.', 'duplicator-pro');
            }

            $repeated_local_storage = $storage->is_path_repeated();
            if ($repeated_local_storage) {
                $error_message = __('A local storage already exists in that folder.', 'duplicator-pro');
            }
        }

        if (!$core_dir_used && !$repeated_local_storage) {
            $storage->save();
        }
