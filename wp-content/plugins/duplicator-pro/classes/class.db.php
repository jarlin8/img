...
class DUP_PRO_DB extends wpdb
{
    ...
    public static function getMySqlDumpPath()
    {
        ...
        //Common Linux Paths
            ...
            $mysqldump = Shell::runCommand('which mysqldump', Shell::AVAILABLE_COMMANDS);

            if (!$mysqldump->isEmpty() && DUP_PRO_U::isExecutable($mysqldump->getOutputAsString())) {
                $paths[] = (!empty($mysqldump)) ? $mysqldump : '';
            }

            $mysqlDumpPath = Shell::runCommand('which mysqldump', Shell::AVAILABLE_COMMANDS);
            $mysqldump = $mysqlDumpPath->getOutputAsString() . "/mysqldump";
