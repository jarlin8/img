...
//|                                              https://www.ssgg.net |
//+------------------------------------------------------------------+
#property link          "https://www.ssgg.net"
      ...
      if(BuyLimitLots < MaxLotSize)
        {
         ...
         if(MaxLotSize <= BuyLimitLots && BuyLimitLots < MaxLotSizeA)
           {
            CurrentBuyLotSize = BuyLimitLots * LotSizeTimes;
           }
         else
            if(MaxLotSizeA < BuyLimitLots && BuyLimitLots < MaxLotSizeB)
              {
               CurrentBuyLotSize = BuyLimitLots + LotSizeAddA;
              }
            else
              {
               CurrentBuyLotSize = BuyLimitLots + LotSizeAddB;
              }
   int BuyTicket = OrderSend(Symbol(), OP_BUYLIMIT, CurrentBuyLotSize, BuyLimitPrice, 3, 0, 0, "GoldLimit", MagicNumber, 0, clrGreen);
   if(BuyTicket < 0)
     {
      ...
      if(SellLimitLots < MaxLotSize)
        {
         ...
         if(MaxLotSize <= SellLimitLots && SellLimitLots < MaxLotSizeA)
           {
            CurrentSellLotSize = SellLimitLots * LotSizeTimes;
           }
         else
            if(MaxLotSizeA < SellLimitLots && SellLimitLots < MaxLotSizeB)
              {
               CurrentSellLotSize = SellLimitLots + LotSizeAddA;
              }
            else
              {
               CurrentSellLotSize = SellLimitLots + LotSizeAddB;
              }

   int SellTicket = OrderSend(Symbol(), OP_SELLLIMIT, CurrentSellLotSize, SellLimitPrice, 3, 0, 0, "GoldLimit", MagicNumber, 0, clrRed);
   if(SellTicket < 0)
     {
